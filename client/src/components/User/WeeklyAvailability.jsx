  ...
  return (
    <div className="flex justify-center w-full ">
      <form className="flex flex-col p-5 w-[90%] md:w-[60%]  max-h-full">
                        ...
                        availability.find((elem) => Object.keys(elem)[0] === Object.keys(eachObj)[0]).time.map((startEndObj, timeIndex) => {
                          return (
                            <div key={timeIndex} className="flex items-center gap-3 w-[80%] ml-4" >
                              <WeeklyAvailabilityInput eachObj={eachObj} timeIndex={timeIndex} availability={availability} startEndObj={startEndObj} position={"start"} />
                              -
                              <WeeklyAvailabilityInput eachObj={eachObj} timeIndex={timeIndex} availability={availability} startEndObj={startEndObj} position={"end"} />
                              <div className="flex justify-center items-center w-7 h-7 hover:bg-gray-200 hover:rounded-full">
                                <svg onClick={(e) => handleDelete(e, Object.keys(eachObj)[0], startEndObj)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
        ...
        <button onClick={(e) => handleSubmit(e)} className="bg-green-400 font-bold text-white rounded-lg w-[30%] md:w-[50%] py-2 my-5 mx-auto">
          Change apply
        </button>
      </form>
