...
const WeeklyAvailability = () => {
  ...
  const displayTimeDropdown = () => {
    setIsDropdownOpen(!isDropdownOpen)
  }

  const handleMethod = (meyhod) => {
    if(meyhod === "delete"){
  ...
  return (
    <form className='flex flex-col shrink p-5 md:w-[60%]'>
      {displayDow.map((eachDow, index) => (
        <div key={index} className='flex justify-between width-full my-4'>
          <div className='flex justify-between w-full'>
            ...
            {selectedDow.find(elem => Object.keys(elem)[0] === eachDow)[eachDow]
              ? (
                <div className='flex items-center gap-3 w-[80%] ml-4 relative'>
                  <div id='1' className='relativ'>
                    <input
                      type="text"
                      value="09:00"
                      onClick={displayTimeDropdown}
                      onChange={(e) => e.target.value}
                      className='border border-gray-700 rounded-lg w-16 h-7 text-center focus:border-green-400'
                    />
                    {isDropdownOpen && <TimeDropdown />}
                  </div>
                  -
                  <div id='2' className='relativ'>
                    <input 
                      type="text"
                      value="17:00"
                      onClick={displayTimeDropdown}
                  ...
                  <svg onClick={() => handleClick("add")} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
      ...
      <button
        type='submit'
        className='bg-green-400 font-bold text-white rounded-lg w-[30%] py-2'
      >
        Change apply
      </button>
    </form>
