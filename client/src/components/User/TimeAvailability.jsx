...
const TimeAvailability = ({ selectDate }) => {
  ...
  const [availableTimeArr, setAvailableTimeArr] = useState([0])
  ...
  const handleMethod = (e, method, deleteIndex) => {
    e.preventDefault()
    if(method === "delete"){
      const filteredArr = availableTimeArr.filter((elem, index) => e[index] !== e[deleteIndex])
      setAvailableTimeArr(filteredArr)
    }else if('add'){
      setAvailableTimeArr(prev => {
        return [...prev, 0]
      })
    }
  }
  ...
  return (
    <>
      <div id="timeAvailability" className="flex w-full items-center justify-center mb-2">
      ...
      <div className='flex justify-between w-[80%] my-5 mx-auto'>
              ...
              return (
                <div key={timeIndex} className='flex items-center gap-3 w-full'>
                  <div className='relativ'>
                    ...
                    {selectedItem === `start+${timeIndex}` && <TimeDropdown position={"start"} from={"daily"} date={eachDateTime.date} />}
                  </div>
                  -
                  <div className='relativ'>
                    <input 
                    ...
                    />
                    {selectedItem === `end+${timeIndex}` && <TimeDropdown position={"end"} from={"daily"} date={eachDateTime.date} />}
                  </div>
                  <div className='flex justify-center items-center w-7 h-7 hover:bg-gray-200 hover:rounded-full'>
                    <svg onClick={(e) => handleMethod(e, "delete", timeIndex)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
        ...
        <button onClick={(e) => handleMethod(e, "add")} className='flex justify-center items-center w-7 h-7 hover:bg-gray-200 hover:rounded-full'>
