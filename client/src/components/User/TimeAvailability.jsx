            ...
            {currentAvailbleTime &&
               currentAvailbleTime.map((eachTimeObj, eachTimeObjIndex) => (
                <div key={eachTimeObjIndex} className='flex items-center gap-3 w-full'>
                  ...
                  -
                  <EachTimeInput position={"end"} eachTimeObjIndex={eachTimeObjIndex} eachTimeObj={eachTimeObj} selectDate={selectDate} selectedItem={selectedItem} displayTimeDropdown={displayTimeDropdown} />
                  
                  <div className='flex justify-center items-center w-7 h-7 hover:bg-gray-200 hover:rounded-full'>
                    <svg onClick={(e) => handleMethod(e, "delete", selectDate, eachTimeObj)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
