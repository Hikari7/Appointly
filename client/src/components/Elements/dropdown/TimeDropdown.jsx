...
const TimeDropdown = ({
  ...
  useEffect(() => {
    if (from === "daily") {
      setStartTimeStyle(
        "flex flex-col bg-white m-4 px-1.5 border-2 border-green-400 rounded-lg w-fit h-[400%] overflow-y-scroll absolute top-[55%] left-[-24.5%] z-50"
      );
      setEndTimeStyle(
        "flex flex-col bg-white m-4 px-1.5 border-2 border-green-400 rounded-lg w-fit h-[400%] overflow-y-scroll absolute top-[55%] left-[-25%] z-50"
      );
    } else {
      setStartTimeStyle(
        "flex flex-col bg-white m-4 px-1.5 border-2 border-green-400 rounded-lg w-fit h-[400%] overflow-y-scroll absolute top-[55%] left-[-25%] z-50"
    ...
    <div className={position === "start" ? startTimeStyle : endTimeStyle}>
      {timeArr &&
        timeArr.map((eachTime, index) => (
          <div
            onClick={() => handleSetTime(eachTime)}
            key={index}
